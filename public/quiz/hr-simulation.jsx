...
window.HRSimulationApp = function ({ simulationData, uiVersion }) {
  ...
  // Analytics: Track Simulation Completion + expose data for quiz wrapper
  React.useEffect(() => {
    if (screen === 'sim_result') {
      ...
      // Expose history and score for the quiz wrapper to save
  ...
  const proceedToNextStep = () => {
    ...
    if (currentStep < steps.length - 1) {
      ...
      // In quiz mode: skip sim_result screen, save results & redirect
      if (window.QUIZ_SKILL_ID) {
        const skillId = window.QUIZ_SKILL_ID;
        const returnUrl = window.QUIZ_RETURN_URL || '/';
      ...
      // Normal mode: show sim_result screen
    ...
    return (
      <div style={{ minHeight: '100vh', background: `linear-gradient(180deg, ${COLORS.bg} 0%, #091620 100%)`, padding: '24px 20px', fontFamily: 'system-ui, -apple-system, sans-serif', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ position: 'sticky', bottom: 0, padding: '20px 0', background: 'transperant', zIndex: 10, marginTop: 'auto' }}>
          <button id="reset-path" onClick={() => window.location.reload()} style={{ width: '100%', padding: '18px', background: COLORS.cta, border: 'none', borderRadius: '14px', cursor: 'pointer', fontSize: '16px', fontWeight: 600, color: '#0D2436', boxShadow: '0 4px 24px rgba(127, 194, 65, 0.3)' }}>Go Back to Profile</button>
        </div>
      </div>
    );
  }
